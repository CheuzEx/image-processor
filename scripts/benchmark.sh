...
for P in $LISTA_P; do
    ...
    SALIDA=$(erl -noshell -pa erlang -eval \
        "image_processor:main([\"$IMG\",\"$OUT\",\"$P\",\"$FILTRO\"]), init:stop()." 2>&1)
    echo "$SALIDA"
    T=$(echo "$SALIDA" | grep -oE 'Tp \(p=[0-9]+\) = [0-9]+' | grep -oE '[0-9]+$')
    if [ -z "$T" ]; then
        echo "  (!) no se pudo medir el tiempo para p=$P, revisar el log anterior"
        continue
    fi
    echo "$P,$T" >> "$CSV"
done
...
echo ""
echo "== Resultados (Tp, Speedup Sp=T1/Tp, Eficiencia Ep=Sp/p) =="
printf "%-6s %-12s %-10s %-10s\n" "p" "Tp(ms)" "Sp" "Ep"
awk -F, 'NR==1{next} NR==2{T1=$2} {
    Sp = T1/$2; Ep = Sp/$1;
    printf "%-6s %-12s %-10.2f %-10.2f\n", $1, $2, Sp, Ep
}' "$CSV"

echo ""
echo "CSV crudo guardado en: $CSV"
