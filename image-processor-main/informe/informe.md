...
| `transpuesta` | Filtro `transponer` (intercambia filas/columnas) | Verbatim, sin ninguna conversión (la matriz ya es lista de listas) |
...
diferencia despreciable (~1%). *(Nota: esa comparación es de una
versión anterior del código, que todavía usaba vectores para el acceso
O(1) a los píxeles. La versión actual de `filtro.rkt` usa listas puras
en todo lugar, sin ningún vector — ver la actualización más abajo.)*

**Actualización — listas puras en vez de vectores:** se migró
`filtro.rkt` a usar únicamente listas (sin `vector`/`list-ref`,
acceso manual con `nth-aux`), para seguir más de cerca el estilo de
`clase.scm`. Se remidió la misma convolución sobre la imagen de
400×300: **925 ms** (contra 205 ms de la versión con vectores, ≈4.5×
más lenta). El total del proceso sigue siendo rápido (~3.3 s para toda
la imagen en una sola región), muy lejos del timeout de 30 s de
Erlang. Es un costo real y medido, no una suposición: se prefirió
fidelidad al estilo de la clase (recursión pura sobre listas) sobre el
rendimiento máximo, y quedó documentado el trade-off con números
concretos en vez de dejarlo como una afirmación sin verificar.
